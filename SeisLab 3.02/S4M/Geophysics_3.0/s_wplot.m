...
% Last updated:September 19, 2015: Eliminate EraseMode from plots
% 
...
%            July 15, 2009: Replace two menu buttons by one menu. Replace
...
for ii=1:ntr
 ...
 %          'EraseMode','normal','LineWidth',param.wiggle_width);
      handles(ii,3)=line(y_data(2:end-1)+location(ii),x_data(2:end-1),'Color',param.wiggle_color, ...
...
for ii=1:ntr
   ...
   if ~isnullvalue  || ~all(isnan(traces(:,ii)))
      ...
      %    'Color',param.wiggle_color,'EraseMode','normal','LineWidth',0.5);
      handles(ii,1)=line(0*traces(:,ii)+location(ii),times, ...
          'Color',param.wiggle_color,'LineWidth',0.5);
      % Plot spikes
